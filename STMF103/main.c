...
int main()
{
  ...
  while(1){
    if(getCountCommand() && getTXcounter()==0){
      nextionEvent();
      }
    if(itsTimeFlag){
...
void updateEEPROM(){
  ...
  if(tailHourBuf%24==0){                                                        //Прошло 24 измерения (144 секунды) и можно положить в буфер дня одно измерение
    dayT1Buf[tailDayBuf] = getSr(24,hourT1Buf);                                 //Кладем среднее от 24 последних измерений
    dayT2Buf[tailDayBuf] = getSr(24,hourT2Buf);
    dayHBuf[tailDayBuf] = getSr(24,hourHBuf);
    dayPBuf[tailDayBuf++] = getSr(24,hourPBuf);

    //ДЕНЬ
    if(tailDayBuf==ANOTHERBUFSIZE){                                             //Буфер заполнился (144 минуты), пора записывать в EEPROM
  ...
  if(tailHourBuf%168==0){                                                       //Прошло 168 измерений (1008 секунд) и можно записать в буфер недели
    ...
    if(tailHourBuf==HOURBUFSIZE){                                               //Буфер часа заполнился, пора записывать
      for(int i=0;i<10;i++){
